...
class Tweetsie {
  ...
  parseTemplate() {
    ...
    this.tweets.forEach((tweet) => {
      ...
      matches.forEach((match) => {
        ...
        let value = eval(`tweet["${varname.split('|')[0].trim().split('.').join('"]["')}"]`);

        if (filters.length > 1) {
          let filter = this[`filter_${filters[1].trim()}`];
  ...
  **/
  filter_auto(date) {
    ...
    return `<time class="tweetsie-auto-date" datetime="${formatteddate}" data-tweetsie-time="${time}">${date}</time>`;
  }
}
